...
export function RoutineSessionsDnd({ routineId, initial }: Props) {
  ...
  function onDragEnd(event: DragEndEvent) {
    ...
    setItems(next);
    startTransition(async () => {
      await reorderRoutineSessionsAction(
        routineId,
  ...
  if (items.length === 0) {
    return (
      <p className="text-sm text-gray-400 py-4 text-center">
        Sin sesiones. Busca y añade una desde el campo de arriba.
      </p>
    );
  }
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
        <SortableContext items={items.map((i) => i.id)} strategy={verticalListSortingStrategy}>
          <ul className="space-y-2">
            {items.map((item) => (
              <SortableRow key={item.id} item={item} routineId={routineId} />
            ))}
          </ul>
        </SortableContext>
      </DndContext>
