-- Amend reorder_items RPC to defer UNIQUE constraints before the atomic UPDATE.
--
-- Why: The companion migration 20260509084405 makes the junction-table UNIQUE
-- constraints DEFERRABLE INITIALLY IMMEDIATE.  Default behaviour is unchanged
-- (INSERT conflicts still fail fast), but the RPC can now call
--   SET CONSTRAINTS ... DEFERRED
-- before the UPDATE FROM jsonb_to_recordset, deferring uniqueness validation
-- to COMMIT.  Without this, row-level enforcement during the UPDATE scan would
-- still abort statements where source and target positions overlap.
--
-- The rest of the function body is identical to 20260508213722_reorder_items_rpc.sql.
...
BEGIN
  ...
  -- Defer the UNIQUE constraints on junction tables for the duration of this
  -- transaction so that the UPDATE FROM jsonb_to_recordset below can swap
  -- order_index values without triggering intermediate violations.
  SET CONSTRAINTS
    session_exercises_session_id_order_index_key,
    routine_sessions_routine_id_order_index_key,
    plan_routines_plan_id_order_index_key
  DEFERRED;

  -- Atomic reorder: UPDATE FROM jsonb_to_recordset in a single statement.
  -- UNIQUE constraint check is deferred to COMMIT — no negative phase needed.
  EXECUTE format(
    'UPDATE %I AS t
...
COMMENT ON FUNCTION public.reorder_items(text, text, uuid, jsonb) IS
  'Atomic reorder for junction tables. Allowed tables: session_exercises, routine_sessions, plan_routines. Validates parent ownership via auth.uid(). Uses SET CONSTRAINTS DEFERRED to allow overlapping position swaps without a negative phase.';
